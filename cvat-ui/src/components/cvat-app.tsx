...
class CVATApplication extends React.PureComponent<CVATAppProps & RouteComponentProps, CVATAppState> {
    ...
    <Text>
        ...
        if (showPlatformNotification()) {
                ...
                content: (
                    <>
                        <Row>
                            <Col>
                                <Text>
                                    {`您使用的浏览器是基于 ${name} ${version} 的 ${engine} 浏览器。` +
                                        ' CVAT 在最新版本的 Chrome 和 Firefox 中进行了测试。' +
                                        ' 我们建议使用 Chrome（或其他基于 Chromium 的浏览器）'}
                                </Text>
                            </Col>
                        </Row>
                        <Row>
            ...
            Modal.warning({
                ...
                content: (
                    <Text>
                        {`${name} v${version} 不支持 CVAT 使用的 API。`}
                        强烈建议您更新浏览器。
                    </Text>
                ),
                onOk: () => stopNotifications(true),
    ...
    public render(): JSX.Element {
        ...
        return (
            <Spin size='large' fullscreen className='cvat-spinner' tip='尝试连接中...' />
        );
    }
}
