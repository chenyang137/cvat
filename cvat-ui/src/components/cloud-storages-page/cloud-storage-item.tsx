...
export default function CloudStorageItemComponent(props: Readonly<Props>): JSX.Element {
    ...
    const onDelete = useCallback(() => {
        ...
        Modal.confirm({
            title: isBulkMode ?
                `删除 ${cloudStoragesToDelete.length} 个选定的云存储` :
                '请确认您的操作',
            content: isBulkMode ?
                '所有选定的云存储将被永久删除。是否继续？' :
                `您即将删除云存储 "${displayName}"。是否继续？`,
                        ...
                        description={(
                            <>
                                ...
                                <Paragraph>
                                    ...
                                    {owner ? <Text type='secondary'>{`用户 ${owner.username}`}</Text> : null}
                                    <Text type='secondary'> 于 </Text>
                                    <Text type='secondary'>{dayjs(createdDate).format('YYYY年MM月DD日')}</Text>
                                </Paragraph>
                                <Paragraph>
